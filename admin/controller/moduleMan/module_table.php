<div class="table-responsive mb-3">
  <table class="table table-striped table-sm caption-top table-bordered">
    ...
    <tbody>
      <?php
      // 构造读取数据列表的SQL语句
      $sql = "SELECT * FROM `module` ORDER BY `orderno` ASC";
      $res = $pdo->query($sql);
      ...
      foreach( $res as $row){
        ...
        echo '<th><a href="\admin\module_edit.php?id='.$row['id'].'">修改 </a>/<a href="\admin\controller\moduleMan\module_delete.php?id='.$row['id'].'" onclick="return delConfirm()"> 删除</a></th>';
        echo '</tr>';
      };
      // 释放内存
      unset($res);
